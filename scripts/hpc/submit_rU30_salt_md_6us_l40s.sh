#!/usr/bin/env bash
# Submit Cranberry MD production for the 04-cranberry rU30 salt conditions.
...
DISORDERED_RUN_ROOT="${DISORDERED_RUN_ROOT:-${OUTPUT_ROOT:-$repo_root/runs/disordered/rU30}}"
if [[ "$DISORDERED_RUN_ROOT" != /* ]]; then
    DISORDERED_RUN_ROOT="$repo_root/$DISORDERED_RUN_ROOT"
fi
PDB_NAME="${PDB_NAME:-rU30_20mM_0_cg_vs_conect.pdb}"
...
for batch in "${batches[@]}"; do
    ...
    for salt in $salts; do
        ...
        if [ ! -f "$pdb_path" ]; then
            ...
            echo "Copy the rU30 input into each salt directory before submitting." >&2
            exit 2
        fi
        pdb_paths+="${pdb_path} "
        run_names+="${salt}mM "
    ...
    batch_job_name="${JOB_NAME:-rU30_md_b${batch}}"
    batch_run_label="${RUN_LABEL:-rU30 salt 6us MD batch ${batch}}"
