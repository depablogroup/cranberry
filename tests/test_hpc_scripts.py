...
PACKED_MD_SCRIPT = REPO_ROOT / "scripts/hpc/submit_md_mps_pack_example_slurm.sh"


def test_md_mps_pack_example_slurm_parses_with_bash() -> None:
    result = subprocess.run(
        ["bash", "-n", str(PACKED_MD_SCRIPT)],
        cwd=REPO_ROOT,
        text=True,
        capture_output=True,
        check=False,
...
def test_md_mps_pack_example_slurm_dry_run(tmp_path: Path) -> None:
    pdb_a = data_path("examples/2ntCG_cg_vs_conect.pdb")
    pdb_b = data_path("examples/157d_cg_vs_conect.pdb")
    output_root = tmp_path / "md_pack"

    result = subprocess.run(
        [
            str(PACKED_MD_SCRIPT),
            "--dry-run",
            "--pdb",
            str(pdb_a),
            "--pdb",
    ...
    assert str(PACKED_MD_SCRIPT) in result.stdout


def test_md_mps_pack_example_slurm_runs_md_directly() -> None:
    script_text = PACKED_MD_SCRIPT.read_text()

    assert '"$CRANBERRY_BIN" md "$pdb_path"' in script_text
    assert "run_cranberry_md_mps_pack.sh" not in script_text
