...
from cranberry.runtime import normalize_platform_properties, openmm_runtime_env_metadata
...
def create_simulation(
    ...
    *,
    ...
    if platform is None and platform_properties is not None:
...
def _build_args(*, pdb_path, model, steps, report_interval, n_record, temperature, salt_concentration, timestep, platform, platform_properties, restart_from, append_outputs, dcd_append, log_append, detailed_append, overwrite, write_minimization_report, periodic, box_padding, enforce_periodic_output, actual_platform):
...
def _check_restart_compatibility(previous_args: dict, run_args: dict, args_path: Path) -> None:
    ...
    warning_fields = ["platform", "platform_properties", "cuda_visible_devices", "cranberry_version", "openmm_version"]
