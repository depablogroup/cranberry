...
`checkpoint.chk` is refreshed at the checkpoint interval during the run and again when the run completes. The checkpoint interval defaults to 10 times the trajectory/log report interval; use `--checkpoint-interval` to tune it explicitly. `log` is the standard OpenMM state log with OpenMM `Progress (%)`, step, time, potential energy, kinetic energy, total energy, temperature, elapsed time, speed, and estimated time remaining. Add `--no-log-progress` to omit the progress column from the log header and rows. `detailed.log` adds force-group decomposition columns, including total potential energy and named CRANBERRY components such as `bond`, `pucker`, `stacking35`, `pairing`, `spline`, and `electrostatic`. `final.pdb` includes `CONECT` records so it can be inspected as a canonical Cranberry PDB.
